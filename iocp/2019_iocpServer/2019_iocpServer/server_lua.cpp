...
#include <chrono>
#include <queue>
#include <concurrent_unordered_map.h>			// 멀티스레드에서 사용가능한 맵
...
concurrent_unordered_map<int, SOCKETINFO*> clients;	
...
void Create_NPC()
{
	...
	for ( int npc_id = NPC_ID_START; npc_id < NPC_ID_START + NUM_NPC; ++npc_id ) {
		...
		clients[npc_id]->x = rand() % 50; //WORLD_WIDTH;
		clients[npc_id]->y = rand() % 50; //WORLD_WIDTH;
